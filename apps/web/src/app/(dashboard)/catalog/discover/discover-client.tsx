...
import { useCatalogDiscovery, useImportDiscovered, type DiscoveredRepo } from '@/hooks/use-catalog';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-text-primary">Auto-Discovery</h1>
            <p className="text-sm text-text-secondary">
              Scan your GitHub repos for catalog-info.yaml files
            </p>
          </div>
        </div>
        <Button onClick={() => discovery.refetch()} disabled={discovery.isFetching}>
      ...
      {!discovery.data && !discovery.isFetching && (
        <div className="rounded-xl border border-surface-3 bg-surface-1 p-12 text-center">
          ...
          <h2 className="text-lg font-medium text-text-primary mb-2">Discover catalog entities</h2>
      ...
      {discovery.data && !discovery.isFetching && (
        <>
          <div className="flex items-center justify-between text-sm text-text-secondary">
            <span>
              Found {discovered.length} repos with catalog files ({scanned} scanned)
            </span>
            <div className="flex items-center gap-3">
              {discovered.length > 0 && (
                <>
          ...
          {importMutation.isSuccess && (
            <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-4 py-3 flex items-center gap-2">
              ...
              <span className="text-sm text-emerald-400">
                Imported {importMutation.data?.data?.imported?.length ?? 0} entities successfully
              </span>
            </div>
          )}
          ...
          {discovered.length === 0 && (
            <div className="rounded-xl border border-surface-3 bg-surface-1 p-8 text-center">
              ...
              <p className="text-xs text-text-secondary mt-1">
                Add a <code className="text-violet-400">catalog-info.yaml</code> to your repo root
                to get started.
              </p>
            </div>
          )}
          ...
          {errors.length > 0 && (
            <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-3">
              <p className="text-xs font-medium text-amber-400 mb-1">Scan errors</p>
              {errors.map((e, i) => (
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-text-primary truncate">{repo.fullName}</span>
            {repo.language && (
          ...
          {repo.description && (
            <p className="text-xs text-text-secondary ml-6 mb-2 truncate">{repo.description}</p>
          )}
